...
type Config struct {
	...
	PenaltyTheta   float64 // reliability penalty exponent (default 1.5)
	AdoptionURef   int     // distinct-client count that saturates Adoption to 100 (default 5)

	// Composite score blend weights (sum must ≈ 1.0). Defaults: 0.40/0.15/0.15/0.20/0.10.
	ScoreWeightReputation float64
	ScoreWeightAdoption   float64
...
func Load() (Config, error) {
	...
	cfg := Config{
		...
		PenaltyTheta:   utils.GetenvFloat("PENALTY_THETA", 1.5),
		AdoptionURef:   utils.GetenvInt("ADOPTION_UREF", 5),

		ScoreWeightReputation: utils.GetenvFloat("SCORE_WEIGHT_REPUTATION", 0.40),
		ScoreWeightAdoption:   utils.GetenvFloat("SCORE_WEIGHT_ADOPTION", 0.15),
		ScoreWeightServices:   utils.GetenvFloat("SCORE_WEIGHT_SERVICES", 0.15),
		ScoreWeightPublisher:  utils.GetenvFloat("SCORE_WEIGHT_PUBLISHER", 0.20),
		ScoreWeightCompliance: utils.GetenvFloat("SCORE_WEIGHT_COMPLIANCE", 0.10),
