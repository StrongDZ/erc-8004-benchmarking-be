...
import (
	...
	consumerapp "erc-8004-benchmarking-be/internal/app/consumer"
...
func main() {
	...
	// --- MongoDB ---
	mongoClient, err := mongoclient.NewClient(ctx, cfg.MongoURI)
	if err != nil {
	...
	if cfg.RedisURL != "" {
		...
		if err != nil {
			log.Printf("consumer: redis disabled (connect failed): %v", err)
		} else {
			redisClient = rc
			defer func() { _ = rc.Close() }()
			log.Printf("consumer: redis connected; realtime channel=%s", cfg.RedisEventsChannel)
		}
	}

	log.Printf("consumer started; queue=%s prefetch=%d", cfg.RabbitMQQueue, cfg.ConsumerPrefetch)

	app := consumerapp.NewApp(consumer, eventsRepo, redisClient, cfg.RedisEventsChannel)
